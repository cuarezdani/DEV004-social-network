...
function ib(a) {
  if ("number" !== typeof a && a && !a.ba) {
    ...
    if (b2 && b2[A])
      ...
      (c = db(b2)) ? (Wa$1(c, a), 0 == c.h && (c.src = null, b2[Xa] = null)) : Pa(a);
    }
  }
}
function fb(a) {
...
const onPostsChange = (callback) => tf(ol(Ja(uh(), "Posts"), dl("date", "desc")), callback);
const addCommentToPost = (postRef, comment) => Xl(Ja(postRef, "Comments"), { comment, date: new Date() });
const addPost = (post) => Xl(Ja(uh(), "Posts"), post);
const deletePost = (docRef) => Zl(Za(uh(), "Posts", docRef));
const getComments = (docRef, callback) => tf(ol(Ja(docRef, "Comments"), dl("date", "desc")), callback);
const updatePost = (docRef, data) => Yl(docRef, data);
const saveUser = (user) => Xl(Ja(uh(), "Users"), user);
const sumLike = (id2, user) => Yl(Za(uh(), "Posts", id2), {
  likes: _f(user)
});
const removeLike = (id2, user) => Yl(Za(uh(), "Posts", id2), {
  likes: wf(user)
});
const registerWithEmail = (email, password, displayName) => {
  const auth2 = getAuth();
  return createUserWithEmailAndPassword(auth2, email, password).then((usercredentials) => updateProfile(usercredentials.user, { displayName }).then(() => usercredentials)).then((usercredentials) => {
...
const signInWithGoogle = () => {
  ...
  return signInWithPopup(auth2, provider).then((result) => {
...
const Register = () => {
  ...
  const frase = document.createElement("h6");
  ...
  const or = document.createElement("strong");
  or.textContent = "or";
  or.className = "or";
...
const Login = () => {
  ...
  buttonContinue.addEventListener("click", async () => {
    ...
    if (emailValue === "" || passwordValue === "") {
      ...
      signInWithPassword(user.email, user.emailPassword).then((userCredential) => {
        userCredential.user;
        window.location.href = "/feed";
      }).catch((error) => {
        const errorCode = error.code;
        ...
        } else if (errorCode === "auth/invalid-password" || errorCode === "auth/wrong-password") {
...
const storage$1 = getStorage();
function addPicture(archivo, idPost, updatePostImage) {
  const refStorage = ref(storage$1, `images/${idPost}`);
  const metadata = {
...
const Feed = () => {
  ...
  const profileIcono = document.createElement("img");
  profileIcono.className = "profileIcono";
  profileIcono.src = "../imagenes/usuario.png";
  const addIcono = document.createElement("img");
  addIcono.className = "addIcono";
  addIcono.src = "../imagenes/mas.png";
  const signOut2 = document.createElement("img");
  signOut2.className = "signOut";
  signOut2.src = "../imagenes/cerrar-sesion.png";
  containerFeed.appendChild(menuIcono);
  menuIcono.append(profileIcono, addIcono, signOut2);
  ...
  document.getElementById("modalPost");
  document.querySelector(".addIcono");
...
const Profile = () => {
  ...
  const signOutProfile = document.createElement("img");
